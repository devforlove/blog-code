...
Computed 패턴은 조회에 필요한 집계 데이터를 미리 계산해 놓는 패턴입니다. 미리 집계 데이터를 계산해 두었기 때문에 조회시에는 따로 계산할 필요가 없습니다. 
예를 들면 게시물에 함께 조회되는 조회수, 댓글 수 또는 영화 정보 화면에서 영화를 본 전체 관객 수, 전체 수익에 대한 정보 등 조회시에 필요한 집계 데이터를 백그라운드에서 미리 계산합니다. 
물론 집계 데이터를 백그라운드에서 계산하면 정확한 숫자는 알 수 없을 것입니다. 하지만 대부분 정확한 숫자는 중요하지 않습니다.

Computed 패턴은 조회에 필요한 계산을 쓰기 작업에서 미리 하기 때문에, 읽기 집약적인 서비스에서 큰 효과가 있습니다. 

![img_2.png](img_2.png)

이 패턴을 사용하면 읽기의 CPU 작업 부하를 줄이고 애플리케이션의 성능을 높일 수 있습니다.

## Extended Reference 패턴 

Extended Reference 패턴은 Embedded Data와 Reference의 단점을 보완한 패턴입니다. 
주문 데이터를 저장하는 컬렉션이 있다고 가정하면, 주문 도큐먼트에는 고객에 대한 모든 정보를 복제하는 대신 자주 액세스하는 필드만 복사합니다. 

이 패턴을 이용하면 다른 컬렉션과 JOIN을 하지 않으면서, 데이터의 중복을 최소화할 수 있습니다. 자주 엑세스하는 필드만 복사하기 때문입니다.

![img_3.png](img_3.png)

하지만 이 패턴은 여전히 데이터 중복이 발생하기 때문에, 변경이 자주 발생하지 않는 데이터를 함께 저장하는 것이 좋습니다. 